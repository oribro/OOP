...
public class FilterFactory {
	 ...
	 * @throws BadFilterWarning if the name or parameters of the filterString
	 * were invalid.
	 */
	...
	public static Filter createFilter(String filterString)
		...
		try{
			if (filterName.equals(GREATER_THAN_FILTER_NAME)){
				filter = new GreaterThanFilter(commandParts[FILTER_FIRST_VALUE
				                                            ]
						);
			}
			else if (filterName.equals(SMALLER_THAN_FILTER_NAME)){
				filter = new SmallerThanFilter(commandParts[FILTER_FIRST_VALUE
				                                            ]
						);
			}
			else if (filterName.equals(BETWEEN_FILTER_NAME)){
				filter = new BetweenFilter(commandParts[FILTER_FIRST_VALUE],
						commandParts[FILTER_SECOND_VALUE]);
			}
			else if (filterName.equals(FILE_FILTER_NAME)){
				filter = new FileFilter(commandParts[FILTER_FIRST_VALUE]);
			}
			else if (filterName.equals(CONTAINS_FILTER_NAME)){
				filter = new ContainsFilter(commandParts[FILTER_FIRST_VALUE]);
			}
			else if (filterName.equals(PREFIX_FILTER_NAME)){
				filter = new PrefixFilter(commandParts[FILTER_FIRST_VALUE]);
			}
			else if (filterName.equals(SUFFIX_FILTER_NAME)){
				filter = new SuffixFilter(commandParts[FILTER_FIRST_VALUE]);
			}
			else if (filterName.equals(WRITABLE_FILTER_NAME)){
				filter = new WritableFilter(commandParts[FILTER_FIRST_VALUE]);
			}
			else if (filterName.equals(EXECUTABLE_FILTER_NAME)){
				filter = new ExecutableFilter(commandParts[FILTER_FIRST_VALUE
				                                           ]);
			}
			else if (filterName.equals(HIDDEN_FILTER_NAME)){
				filter = new HiddenFilter(commandParts[FILTER_FIRST_VALUE]);
			}
		...
		// The filter could not be created as requested. Set warning on and
		// use default all filter.
